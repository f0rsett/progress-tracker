...
# заглушаем предупреждения про отсутствующие глифы (🔥, 🏆 и т.п.)
...
# ---------- пути к данным ----------
# Если запущено как .exe (PyInstaller) — берём папку, где лежит .exe.
# Если как обычный .py — берём папку, где лежит .py.
...
_cfg = load_config()
DATA_DIR = _cfg.get("data_dir") or DEFAULT_DATA_DIR
# если сохранённый путь недоступен — откатываемся на путь по умолчанию
if not os.path.isdir(DATA_DIR):
    print(f"Внимание: папка {DATA_DIR} недоступна, использую {DEFAULT_DATA_DIR}")
...
def set_data_dir(path: str):
    """Сменить папку с данными и запомнить в конфиге."""
...
print("Игры:              ",
      sorted(d for d in os.listdir(PROFILES_DIR)
...
def sanitize_name(name: str) -> str:
    """Приводит строку к безопасному имени файла/папки."""
    name = name.strip()
...
def list_profiles():
    """Возвращает список (game, nick) для всех найденных баз."""
    result = []
    if not os.path.isdir(PROFILES_DIR):
        return result
    for game in sorted(os.listdir(PROFILES_DIR)):
        game_path = os.path.join(PROFILES_DIR, game)
        if not os.path.isdir(game_path):
            continue
        for f in sorted(os.listdir(game_path)):
            if f.endswith(".db"):
...
class DB:
    ...
    def add_goal(self, name, target, gtype, is_daily=0):
        gid = str(uuid.uuid4())
        self.conn.execute(
            "INSERT INTO goals(id,name,target,type,created_at,is_daily) VALUES (?,?,?,?,?,?)",
    ...
    def add_kda_entry(self, gid, k, d, a):
        self.conn.execute(
            "INSERT INTO entries(goal_id,k,d,a,created_at) VALUES (?,?,?,?,?)",
            (gid, k, d, a, datetime.now().strftime("%Y-%m-%d %H:%M")))
        self.conn.commit()

    def add_time_entry(self, gid, sec):
        self.conn.execute(
            "INSERT INTO entries(goal_id,time_sec,created_at) VALUES (?,?,?)",
            (gid, sec, datetime.now().strftime("%Y-%m-%d %H:%M")))
        self.conn.commit()
...
class LoginWindow(ctk.CTk):
    def __init__(self):
        ...
        self.game_entry = ctk.CTkEntry(self, placeholder_text="Например: CS2",
                                       width=340, height=40)
    ...
    def choose_folder(self):
        ...
        messagebox.showinfo(
            ...
            "Профили будут сохраняться здесь.",
            parent=self)
    ...
    def login(self):
        ...
        if not raw_game:
            messagebox.showerror("Ошибка", "Введи название игры", parent=self)
            return
        if not raw_nick:
            messagebox.showerror("Ошибка", "Введи ник", parent=self)
            return
        ...
        if not game:
            messagebox.showerror(
                "Ошибка",
                "Название игры содержит только недопустимые символы",
                parent=self)
            return
        if not nick:
            messagebox.showerror(
                "Ошибка",
                "Ник содержит только недопустимые символы",
                parent=self)
            return
...
class App(ctk.CTk):
    def __init__(self, game: str, nickname: str):
        ...
        ctk.CTkButton(self.sidebar, text="+ Новая цель",
                      command=self.add_goal_dialog).pack(
            pady=(0, 10), padx=20, fill="x")

        # футер — игра/ник, кнопки, путь
    ...
    def get_date_range(self, goal=None):
        if goal is not None and goal["is_daily"]:
    ...
    def filter_label(self, goal=None):
        if goal is not None and goal["is_daily"]:
            return "за сегодня"
        return {"all": "за всё время", "today": "за сегодня",
    ...
    def create_goals_frame(self):
        ...
        for g in goals:
            ...
            self.create_goal_card(g, done, streak)

    def create_goal_card(self, goal, done, streak=None):
    ...
    def build_filter_bar(self, goal):
        ...
        options = ["Все время", "Сегодня", "7 дней", "30 дней", "Свой период"]
        key_map = {"Все время": "all", "Сегодня": "today", "7 дней": "week",
                   "30 дней": "month", "Свой период": "custom"}
        ...
        opt = ctk.CTkOptionMenu(row, values=options, command=on_change, width=150)
        ...
        if goal["type"] == "kda":
            ...
            def add():
                try:
                    ...
                    messagebox.showerror("Ошибка",
                                         "Введи целые числа для K / D / A")
                    return
                self.db.add_kda_entry(goal["id"], k, d, a)
                self.refresh()
            ...
            def add():
                try:
                    ...
                    return
                self.db.add_time_entry(goal["id"], secs)
                self.refresh()
    ...
    def build_entries_section(self, goal):
        ...
        ctk.CTkLabel(header, text="Записи",
                     font=ctk.CTkFont(size=17, weight="bold")).pack(side="left")

        if goal["is_daily"]:
            def on_toggle(choice):
                self.daily_show_all = (choice == "Все")
                self.after(0, self.refresh_main)

            opt = ctk.CTkOptionMenu(
                header, values=["Все", "Только сегодня"], width=170,
                command=on_toggle)
            opt.set("Все" if self.daily_show_all else "Только сегодня")
            opt.pack(side="right")

        if goal["is_daily"] and not self.daily_show_all:
            today = datetime.now().strftime("%Y-%m-%d")
            entries = self.db.list_entries(goal["id"], today, today)
        elif goal["is_daily"]:
            entries = self.db.list_entries(goal["id"])
        else:
            dfrom, dto = self.get_date_range(goal)
    ...
    def _draw_heatmap(self, goal):
        ...
        def color_for(cnt):
            ...
            if goal["is_daily"]:
                ...
                ratio = min(cnt / max(max_count, 1), 1.0)
    ...
    def add_goal_dialog(self):
        ...
        name_entry = ctk.CTkEntry(dlg, placeholder_text="Например: Deathmatch")
        ...
        ctk.CTkRadioButton(tf, text="KDA (K/D/A)", variable=type_var,
                           value="kda").pack(side="left", padx=5)
        ctk.CTkRadioButton(tf, text="Время", variable=type_var,
                           value="time").pack(side="left", padx=15)

        daily_var = ctk.BooleanVar(value=False)
        ctk.CTkCheckBox(
            dlg,
            text="🔁 Ежедневная цель (прогресс сбрасывается каждый день)",
            variable=daily_var).pack(pady=(20, 5), padx=20, anchor="w")

        def create():
            name = name_entry.get().strip()
            if not name:
                messagebox.showerror("Ошибка", "Введи название", parent=dlg)
                return
            try:
        ...
        ctk.CTkButton(dlg, text="Создать", command=create).pack(pady=25)
